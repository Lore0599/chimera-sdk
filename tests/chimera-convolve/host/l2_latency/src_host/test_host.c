...
uint32_t measure_l2_read_latency() {
    volatile uint32_t* l2_ptr = (volatile uint32_t*) MEM_RD_START;
    uint32_t start, end;
...
int main(void) {
    ...
    // test_cluster_cfg_t* test_cfg_ptr = &test_cfg;
    ...
    do {
        ...
        for (int id = 0; id < test_cfg.clusters; id++) {
            ...
            if (test_cfg.mode != TEST_MODE_DUTCTL) {
                ...
                // printf_log("Num Cores: %d\n", _chimera_numCores[clusterId]);
                // printf_log("Offloading to all cores in cluster %d from hartid %d to hartid %d\n",
                            // clusterId, hartId_min, hartId_max);
        ...
        for (int id = 0; id < test_cfg.clusters; id++) {
            test_cluster_args_t *arg = test_cfg.args[id];
            volatile int32_t *byp_wide_ptr;
            byp_wide_ptr = (volatile int32_t *)(SOC_CTRL_BASE +
                                                CHIMERA_WIDE_MEM_CLUSTER_4_BYPASS_REG_OFFSET);
            *byp_wide_ptr = 1; // Bypass wide memory for cluster 4
        ...
        // Host code to access L2

        printf("[");
        while(1) {
            uint32_t l2_read_cycles = measure_l2_read_latency();
            float avg_latency = l2_read_cycles / (float)(1*11);

            // printf("Total Latency:: %u cycles\n", l2_read_cycles);
            printf("%u, ", l2_read_cycles);
        }
